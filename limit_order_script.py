...
if config_file and os.path.exists(config_file):
    ...
    secret_name = os.environ.get('AWS_SECRET_NAME', 'limit-order-bot/wallet-key')
    
    fetched_key = get_secret_from_aws(secret_name, aws_region)
    if fetched_key:
        globals()['PRIVATE_KEY'] = fetched_key
        print("✅ PRIVATE_KEY loaded from AWS Secrets Manager")
    else:
        print("⚠️  Could not load PRIVATE_KEY from Secrets Manager")
        # Check if PRIVATE_KEY exists in config file
        if 'PRIVATE_KEY' in globals() and globals()['PRIVATE_KEY']:
            print("   Using PRIVATE_KEY from config file (fallback)")
        else:
            print("❌ No PRIVATE_KEY available! Cannot proceed.")
            sys.exit(1)
else:
